...
- Technical gate: **PASS**; research status: **DISCOVERY_ONLY** (guide 3.5: no
  ...
  migration table).
- Branch `mode4-corrective`, guide RA-GUIDE-1.0 (section 9), study `regime_time_edge_ra_v1`, run `ra05-20260918T201236Z-de11db62`.
- Pilot cell A-SC/BTCUSDT, window
  2021-01-01 to 2021-04-01 (90-day window, 45-day rolling train memory (registered: 180), 8 trials/cutoff (registered: 50) -- same precedent as RA-02/03/04's phase-owned real-but-small experiments; the full registered contract is not spent proving plumbing here).
...
- F-06 (28 accepted triggers, median gap ~28.17d, min-gap28, `time_edge/schedule.py`): re-audited
  on real emissions in this window -- 1 accepted trigger(s) via the
  F-06-cited function (confirmations=2), vs 2 via the function that
...
- Route `event` (real per-fill native QuantBT account, RF-04 precedent for A-SC), seed
  `20260918`, 8 trials/cutoff.

## 4. Exit-gate matrix
| Gate | Expected | Actual | Evidence ref | Status |
|---|---|---|---|---|
| G05-EXEC | PASS | PASS | ra05-20260918T201236Z-de11db62/phase_manifest.json | PASS |
| G05-TRACE | PASS | PASS | ra05-20260918T201236Z-de11db62/phase_manifest.json | PASS |
| G05-LEARN | PASS | PASS | ra05-20260918T201236Z-de11db62/phase_manifest.json | PASS |
| G05-SCOPE | PASS | PASS | ra05-20260918T201236Z-de11db62/phase_manifest.json | PASS |
| G05-MANIFEST | PASS | PASS | ra05-20260918T201236Z-de11db62/phase_manifest.json | PASS |
...
- CAL_MATCHED's cadence forecast used a development prefix DISJOINT from the evaluation window
  (real emissions outside `[2021-01-01, 2021-04-01)`), never the
  window's own realized trigger count -- 747 development days,
  16 triggers, rate 0.02142/day ->
  predicted 1.93 triggers in-window -> menu choice
  **56 days** (menu [28, 56, 90, 180]).
...
- M4_CAL_vs_M4_CAL_MATCHED: 0/2 folds diverged (no)
- M4_CAL_vs_M4_REGIME: 1/2 folds diverged (YES)
- M4_CAL_MATCHED_vs_M4_REGIME: 1/2 folds diverged (YES)
- **Descriptive returns (point estimates only, no CI/claim-gate -- RA-07 scope):**

| arm | total return | fills | folds | admitted/kept |
|---|---|---|---|---|
| STATIC | 4.3416% | 171 | 1 | 1/0 |
| M4_CAL | 3.3837% | 274 | 2 | 2/0 |
| M4_CAL_MATCHED | 2.6339% | 290 | 2 | 2/0 |
| M4_REGIME | 4.2967% | 160 | 3 | 2/1 |
