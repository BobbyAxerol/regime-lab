# FP-10 — Freeze, replay and final handoff (fp10-20260924T205735Z-49161e47)

- run_dir: /root/bobby/pool_alpha/lab_regime_model_quantbt/evidence/forward_persistence_fp_v1/fp10-20260924T205735Z-49161e47
- started_at: 2026-09-24T20:57:35.364616+00:00
...
- fresh replay run: `evidence/forward_persistence_fp_v1/fp09-20260924T205736Z-36daae3f`
...
- cache provenance (informational only, not gated — not gated: a MISS->HIT transition here is EXPECTED and is itself evidence the replay found and reused the original run's exact computation, never required to 'match' literally):

| field | original | replay |
|---|---|---|
| SELECTOR_FIXED_CAL.cache_event | HIT | HIT |
| SELECTOR_CAL_MATCHED.cache_event | MISS | HIT |
| SELECTOR_REGIME_TIMING.cache_event | HIT | HIT |
...
- this phase's own wall time: 67.81s (freeze manifest build + one real FP-09 replay call + artifact index scan)

## Permitted conclusions
- Technical: the freeze manifest's every hashed source file matches what is on disk right now (FP10-G-FREEZE); the replay reproduced every compared economic output exactly (all_match=True, FP10-G-REPLAY), explicitly disclosed as NOT an independent confirmation; data exposure is stated explicitly, outer_evaluation stays untouched (FP10-G-EXPOSURE).
