#!/usr/bin/env python3
"""Build FP-08's cell-1 (A-SC/BTCUSDT) D2 age-curve analysis (guide
10.3/FP08.2). ZERO new engine calls: FP-07's own three continuous accounts
already cover every anchor's own frozen-params segment uninterrupted
through the account's own end (2023-12-31) -- this script reloads the SAME
real frame and re-calls fp.evaluator.run_deployment with the EXACT same
schedules FP-07 used, which a content-addressed cache HIT returns without
ever re-running the engine (guide FP08.2: "Tai dung first-horizon path neu
exact contract/prefix parity"). Every HIT is asserted, not assumed -- a
MISS here would mean an unexpected, expensive recomputation and the script
refuses rather than silently paying for it.

Anchors: guide FP08.2 item 1, every origin where an arm admitted a REAL
selection (fp.d2_continuation.anchors_from_selections) -- 12 for
A_STOCK_CAL (always admits), 3 each for B_FP_PERSISTENCE/C_FP_CONTEXT
(FP-07's own measured real admission count, identical for both per the
already-disclosed C==B degenerate finding).

Usage:
  lab_venv/bin/python scripts/build_fp08_cell1_d2.py
"""
...
def main() -> int:
    ...
    cache = ComputeCache(LAB, "fp04", cache_root=CACHE_ROOT)   # same namespace FP-07 published to
    ...
    for arm in ls.ARMS:
        payload, event = run_deployment(cache, LAB, ALPHA_ID, frame, schedules[arm],
                                        ...
                                        economics=economics, producer="fp08-cell1-d2-reuse")
        if event["status"] != "HIT":
            raise Cell1D2Error(f"{arm}: expected a real cache HIT reusing FP-07's own computation, "
                              f"got {event['status']} -- refusing to pay for an unexpected "
                              "recomputation silently")
    ...
    # Context at each origin's own ready time -- real disk reads, zero new engine calls.
    context_by_origin = {}
    for origin in origins_sorted:
        is_frame = sb.load_origin_is_frame(ALPHA_ID, origin)
    ...
    out = {
        "schema": "regime_lab.fp08_cell1_d2.v1",
        "cell": "cell1_primary", "alpha_id": ALPHA_ID, "symbol": SYMBOL,
        "source_run_dir": str(FP07_RUN_DIR),
        "engine_calls_new": 0,
        "note": "every arm's continuous account reused via a real cache HIT on FP-07's own already"
               "-computed run_deployment payload -- asserted (event.status == 'HIT'), not assumed",
        "records": records,
    }
